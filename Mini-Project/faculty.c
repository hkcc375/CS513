...
void add_new_course( int clientSocket, int faculty_id )
{
	...
	int fileDescriptor_course =
	    open( "course.txt", O_CREAT | O_WRONLY, 0777 );
	if( fileDescriptor_course == -1 )
		perror( COURSE_FILE_OPEN_FAILED );
	else
	{
		int fileDescriptor_faculty =
		    open( "faculty.txt", O_CREAT | O_RDWR, 0777 );
		...
		else
		{
			...
			int retval = read_faculty_record(
			    fileDescriptor_faculty, &f_record, faculty_id,
			    sizeof( struct faculty ) );
			if( retval == 1 )
			{
				if( f_record.no_of_offering_courses <
					...
					if( no_of_courses < MAX_COURSES )
					{
						...
						write(
						    ...
						    sizeof(
						        ENTER_COURSE_NAME ) );
						sleep( 1 );
						int read_course_name_bytes =
						    read( clientSocket,
						          read_buffer, 512 );
						if( read_course_name_bytes ==
						    -1 )
							perror( "Could not "
							        "read course "
							        "name." );
						read_buffer
						    [read_course_name_bytes -
						     1] = '\0';
						memset( new_course.course_name,
						        0, NAME_LENGTH );
						...
						int read_course_department_bytes =
						    read( clientSocket,
						          read_buffer, 512 );
						if( read_course_department_bytes ==
						    -1 )
							perror( "Could not "
							        "read course "
							        "department." );
						read_buffer
						    [read_course_department_bytes -
						     1] = '\0';
						memset(
						    new_course.department, 0,
						    DEPARTMENT_NAME_LENGTH );
						strcpy( new_course.department,
						        read_buffer );
						memset( read_buffer, 0, 512 );
						...
						write(
						    clientSocket,
						    ENTER_COURSE_CREDITS,
						    sizeof(
						        ENTER_COURSE_CREDITS ) );

						sleep( 1 );

						int read_course_credits_bytes =
						    read( clientSocket,
						          read_buffer, 512 );
						if( read_course_credits_bytes ==
						    -1 )
							perror( "Could not "
							        "read course "
							        "credits." );
						read_buffer
						    [read_course_credits_bytes -
						     1] = '\0';
						int credits =
						    atoi( read_buffer );
						...
						int read_course_total_seats_bytes =
						    read( clientSocket,
						          read_buffer, 512 );
						if( read_course_total_seats_bytes ==
						    -1 )
							perror( "Could not "
							        "read course "
							        "total no of "
						...
						write(
						    clientSocket,
						    ENTER_COURSE_TOTAL_AVAILABLE_SEATS,
						    sizeof(
						        ENTER_COURSE_TOTAL_AVAILABLE_SEATS ) );
						sleep( 1 );
						int read_course_total_no_of_available_bytes =
						    read( clientSocket,
						          read_buffer, 512 );
						if( read_course_total_no_of_available_bytes ==
						    -1 )
							perror( "Could not "
							        "read course "
							        "total no of "
							        ...
							        "seats." );
						read_buffer
						    [read_course_total_no_of_available_bytes -
						     1] = '\0';
						int total_no_of_available_seats =
						    atoi( read_buffer );
						new_course
						    .number_of_available_seats =
						    total_no_of_available_seats;
						memset( read_buffer, 0, 512 );
						...
						// Course ID
						memset( new_course.course_id, 0,
						        COURSEID_LENGTH );
						snprintf( new_course.course_id,
						          COURSEID_LENGTH,
						          "CC%03d",
						          no_of_courses );
						...
						// faculty
						strcpy(
						    &f_record.offering_courses
						         [f_record
						              .no_of_offering_courses -
						          1][COURSEID_LENGTH],
						...
						write_faculty_record(
						    fileDescriptor_faculty,
						    &f_record, faculty_id,
						    sizeof( struct faculty ),
						    1 );
						...
						write(
						    clientSocket,
						    FACULTY_RECORD_MODIFIED,
						    sizeof(
						        FACULTY_RECORD_MODIFIED ) );
						write(
						    1, FACULTY_RECORD_MODIFIED,
						    sizeof(
						        FACULTY_RECORD_MODIFIED ) );
						sleep( 1 );
						write(
				...
				else
				{
					write(
					    clientSocket,
					    CANNOT_ADD_COURSE_FACULTY,
					    sizeof(
					        CANNOT_ADD_COURSE_FACULTY ) );
					write(
					    1, CANNOT_ADD_COURSE_FACULTY,
					    sizeof(
					        CANNOT_ADD_COURSE_FACULTY ) );
				}
			}
