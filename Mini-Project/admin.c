		...
		else
		{
			...
			if( retval == 1 )
			{
				...
				write_student_record( fileDescriptor, &record,
				                      student_id,
				                      sizeof( student ), 1 );
				write( clientSocket, STUDENT_RECORD_MODIFIED,
...
void admin_menu_handler( int clientSocket )
{
	...
	while( condition )
	{
		...
		else
		{
			...
			if( userChoice == 0 )
			{
				write( 1, ADMIN_NO_CHOICE_SERVER,
				       sizeof( ADMIN_NO_CHOICE_SERVER ) );
				write( clientSocket, ADMIN_NO_CHOICE_CLIENT,
				       sizeof( ADMIN_NO_CHOICE_CLIENT ) );
			}
			else
			{
				switch( userChoice )
				...
				default:
					write(
					    clientSocket,
					    ADMIN_INCORRECT_CHOICE_CLIENT,
					    sizeof(
					        ADMIN_INCORRECT_CHOICE_CLIENT ) );
					write(
					    1, ADMIN_INCORRECT_CHOICE_SERVER,
					    sizeof(
					        ADMIN_INCORRECT_CHOICE_SERVER ) );
					break;
				}
			}
		}
