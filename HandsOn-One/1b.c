...
// The command to create a hard link using shell command is ln <path to file/folder to be linked> <path of the link to be created>
	...
	else
	{
		...
		else
		{
			...
			if(link(fileName, "hard_link_file") < 0)
				perror("There was an error while creating a hard link. \n");
			else
				printf("The hard link was created successfully. \n");
		}
	}
}
